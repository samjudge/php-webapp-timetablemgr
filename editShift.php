...
<body>
	...
	<form method="POST" onsubmit="return(formValidation());" action="shiftEditSubmission.php">
		
		<input name="shiftId" style="visibility:hidden" value="<?php echo $sId ?>">
		<input name="userId" style="visibility:hidden" value="<?php echo $uId ?>">
		<br/>
		<?php
			echo "Staff Name : <select name='username' id='username'>";
			$result = $connection->query("SELECT username,userId FROM users WHERE siteId = $siteId");
		...
		<br/>
		<br/>
		Start Time : <input name="start" type="text" value="<?php echo $startTime ?>">
		<br/>
		<br/>
		End Time : <input name="end" type="text" value="<?php echo $endTime ?>">
		<br/>
		<br/>
